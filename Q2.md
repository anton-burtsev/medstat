1. **Создать таблицу** актуальных версий organization_versions_actual. Теоретически, лучше явно содать таблицу с перечислением атрибутов, а потом заполнить INSERT-SELECT-ом.
...
2. **Создать триггеры**. На любое изменение перевычисляем актуальную версию за год. Если ничего не вычислилось - просто удаляем вычисленное ранее.
...
3. **Создать индексы**. Тут нужно в каждом запросе разбираться, что нужно кроме organization_versions_actual
...
4. **Исправить запрос**. Хинтов в постгресе нет, поэтому подкручиваем через SET LOCAL. Транзакция нужна для того, чтобы ограничить скоуп для SET LOCAL, иначе они могут примениться к другим запросам и всё сломать. Как вариант можно применить https://github.com/ossc-db/pg_hint_plan, но с ним нужно разбираться, вдруг его нет импортозамещённого... Так же важно указывать **not materialized**, чтобы постгрес мог мёржить SQL и строить оптимальный план. Cкорее всего, отсутствие not materialized одна из причин дикого трафика на диск.
